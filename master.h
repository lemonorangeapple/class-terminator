...
void Suspend_service() {
    if (main_pid != 0) {
        ...
        MessageBoxA(NULL, "未找到进程", "Class-Terminator", MB_OK);
    }
}
...
void Check_Class_service() {
    FindProcessPid(processName, main_pid);
    if (main_pid == 0) {
        MessageBoxA(NULL, "未找到进程", "Class-Terminator", MB_OK);
    }
    else {
        MessageBoxA(NULL, "成功找到进程", "Class-Terminator", MB_OK);
    }
}
